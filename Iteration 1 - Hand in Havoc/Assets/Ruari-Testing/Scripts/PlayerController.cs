...
public class PlayerController : MonoBehaviour
{
    ...
    IEnumerator PlayerHit()
    {
        for (int i = 0; i <= 5; i++)
        {
            ...
            foreach (GameObject part in playerArt)
            {
                part.GetComponent<SpriteRenderer>().color = new Color(part.GetComponent<SpriteRenderer>().color.r, part.GetComponent<SpriteRenderer>().color.g, part.GetComponent<SpriteRenderer>().color.b, 255);
            }
            yield return new WaitForSeconds(0.1f);
        }
    }
