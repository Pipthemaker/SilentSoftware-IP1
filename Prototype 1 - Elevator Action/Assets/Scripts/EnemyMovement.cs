...
public class EnemyMovement : MonoBehaviour
{
	...
	void Update ()
    {
        ...
        switch (mode)
        {
            ...
            case 2: //Enemy finds the stairs to the level up and uses them
                ...
                {
                    gameObject.transform.position = Vector2.MoveTowards(gameObject.transform.position, StairBottoms[0].transform.position, moveSpeed * Time.deltaTime);
            ...
            case 3: //Enemy finds the stairs to the level down and uses them
                ...
                {
                    gameObject.transform.position = Vector2.MoveTowards(gameObject.transform.position, StairTops[0].transform.position, moveSpeed * Time.deltaTime);
            ...
            case 4: //Enemy Randomly moves About Floor
                ...
                {
                    gameObject.transform.position = Vector2.MoveTowards(gameObject.transform.position, new Vector2(targetPosX, transform.position.y), moveSpeed * Time.deltaTime);
                }
                break;
        }
    ...
    public void FindEnvironment()
    {
        ...
        for (int i = 0; i < parentFloor.transform.childCount; i++)
        {
            ...
            if (child.name == "StairsUp")
            {
                StairBottoms.Add(child.gameObject);
            }
            if (child.name == "StairsDown")
            {
